...
async function fetchPage(username, folder, page, externalSignal) {
...
export async function fetchAllReleases(username, folder = 0, onProgress, signal) {
  let page = 1;
  let totalPages = null;
  let allReleases = [];
  ...
  while (true) {
    ...
    while (true) {
      try {
        result = await fetchPage(username, folder, page, signal);
      } catch (err) {
